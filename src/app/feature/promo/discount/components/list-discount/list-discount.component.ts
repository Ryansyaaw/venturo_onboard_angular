...
export class ListDiscountComponent implements OnInit {
  ...
  toggleDiscountStatus(customerId: string, promoId: number): void {
  ...
  isDiscountApplied(customerId:string, promoId: number): boolean {
  ...
  getDiscountsPromo(status = "diskon") {
    this.promoService.getPromo({ status: status }).subscribe(
      (res: any) => {
  ...
  reloadTable() {
    this.customerService.getCustomers(this.filter.id).subscribe((res: any) => {
